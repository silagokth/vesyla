...
RUN pacman -Syu --noconfirm python python-pip cmake gcc make boost protobuf && \
    ...
    pip3 install --break-system-packages ortools protobufprotobuf==4.24.2 pyinstaller verboselogs coloredlogs numpy matplotlib binarytree sympy regex lark uuid svglib svg reportlab && \
