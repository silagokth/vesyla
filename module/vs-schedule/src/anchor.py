...
def add_anchor(name: str, expr: str, anchor_dict: dict):
    if name in anchor_dict:
        logging.error("Name "+name+" is already defined, choose another name for the anchor")
        exit(1)
    anchor_dict[name] = expr

def add_anchor_to_timing_var_dict(anchor_dict: dict, op_table, event_dict_table, node_map, duration_table, timing_var_dict):
    for name in anchor_dict:
        expr = anchor_dict[name]
        if name in timing_var_dict:
            logging.error("Name "+name+" is already defined, choose another name for the anchor")
            exit(1)
        timing_var_dict[name] = slice_name_translation(expr, op_table, event_dict_table, node_map, duration_table, timing_var_dict)

def slice_name_translation(slice_name: str, op_table, event_dict_table, node_map, duration_table, timing_var_dict) -> str:
    ...
    parent_R_nodes = [ x for x in parent_nodes if isinstance(node_map[x.value], operation.RepeatitionOperator)]

    if len(parent_R_nodes) != len(slices_list):
            logging.error("The number of slices in "+slice_name+" does not match the number of R operators outside the event")
            exit(1)
    ...
    for i in range(len(slices_list)):
            ...
            if slice >= iter:
                logging.error("The Slice "+str(i)+" in "+name+" (="+str(slice)+") is out of range")
                exit(1)
            expr = str(simplify(expr + " + ((" + duration_table[op_name][parent_R_nodes[i].left.value] + ")+" + delay_var + ") * " + str(slice)))
    return expr