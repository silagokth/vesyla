...
def convert_operation_event_to_trace_event_list(op_event, counter, op2pid_map):
    ...
    return [TraceEvent(name, cat, "B", op_event.time, pid, tid),
...
def create_trace_file_from_event_list(event_list, filename):
    ...
    for op_event in op_event_list:
        converted_list = convert_operation_event_to_trace_event_list(op_event, counter, op2pid_map)
...
def create_event_list_from_btree(node: Node, op_name, time, node_map, start_time_table, duration_table):
    event_list = []
    if node is None:
        return event_list
    ...
    while current is not None:
        ...
        if isinstance(node_map[current.value], operation.Event):
            ...
            parent_operator_node_list = [x for x in parent_list if isinstance(node_map[x.value], operation.RepeatitionOperator)]
            ...
            all_indices = [list(x) for x in itertools.product(*[range(i) for i in iter_list])]
        ...
        while parent_list and (not parent_list[-1].right or parent_list[-1].right == current):
