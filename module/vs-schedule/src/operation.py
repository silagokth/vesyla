    ...
    for op_name in op_table:
        ...
        for node in sequence:
            ...
            if isinstance(node_map[id], RepeatitionOperator):
                duration_dict[id] = (
                    "("
                    + duration_dict[node.left.value]
                    + ") * ("
                    + str(node_map[id].iter)
                    + ") + ("
                    + node_map[id].delay_var
                    + ") * ("
                    + str(node_map[id].iter)
                    + " - 1)"
                )
            ...
            if isinstance(node_map[id], TransitionOperator):
                duration_dict[id] = (
                    ...
                    + node_map[id].delay_var
                    + ") + ("
                    + duration_dict[node.right.value]
                    + ")"
                )
                duration_dict[id] = str(simplify(duration_dict[id]))
        ...
        for node in sequence:
            ...
            if node.right is not None:
                start_dict[node.right.value] = str(
                    simplify(
                        "("
                        + duration_dict[node.left.value]
                        + ") + ("
                        + start_dict[node.left.value]
                        + ") + ("
                        + node_map[node.value].delay_var
                        + ")"
                    )
                )
    ...
    for op_name in op_table:
        ...
        for event_name in event_dict:
            if event_name in timing_var_dict:
                ...
                exit(1)
            timing_var_dict[event_name] = str(
                simplify(
                    "("
                    + op_name
                    + ")+("
                    + start_time_table[op_name][event_dict[event_name]]
                    + ")"
                )
            )
