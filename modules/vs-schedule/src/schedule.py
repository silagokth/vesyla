...
def parse(
    file_path: str, op_table, anchor_dict, constraint_list, node_map, counter
) -> None:
    with open(file_path, "r") as file:
        ...
        for line in lines:
            ...
            if pattern.match(line):
                ...
                counter = operation.add_operation(
                    op_name, expr, op_table, node_map, counter
                )
                continue
...
def schedule(model_file, output_dir):
    ...
    constraint_list = []
    counter = parse(
        model_file, op_table, anchor_dict, constraint_list, node_map, counter
    )
    [start_time_table, duration_table] = operation.calculate_timing_properties(
        op_table, node_map
    )
    event_dict = operation.extract_event_dict(op_table, node_map)
    timing_variable_table = {}
    operation.add_var_to_timing_var_dict(
        op_table, node_map, start_time_table, timing_variable_table
    )
    anchor.add_anchor_to_timing_var_dict(
        anchor_dict,
        op_table,
        event_dict,
        node_map,
    ...
    [latency, solution] = solver.solve(
        timing_variable_table, constraint_list, op_table, duration_table, 2**32 - 1
    )
    ...
    logging.info(
        "Timing table saved to " + os.path.join(output_dir, "timing_table.json")
    )
    ...
    for op_name in op_table:
        event_list = trace.create_event_list_from_btree(
            op_table[op_name],
            op_name,
            solution[op_name],
            node_map,
            start_time_table[op_name],
            duration_table[op_name],
        )
        all_events.extend(event_list)

    trace.create_trace_file_from_event_list(
        all_events, os.path.join(output_dir, "trace.json")
    )
